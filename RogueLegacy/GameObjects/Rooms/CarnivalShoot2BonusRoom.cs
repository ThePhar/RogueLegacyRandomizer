...
{
    private readonly int                m_numTries = 5;
    ...
    public override void Initialize()
    {
        ...
        {
            if (current is WaypointObj) m_elf.X = current.X;
        ...
        {
            if (current2.Name == "Floor") m_elf.Y = current2.Y - (m_elf.Bounds.Bottom - m_elf.Y) - 2f;

            if (current2.Name == "GatePosition") rectangle = current2.Bounds;
        }
    ...
    public void GiveReward(int gold)
    {
        ...
        Game.PlayerStats.Gold += (int) (gold * RandomizerData.GoldGainMultiplier);
        Tween.By(m_gate, 0.5f, Quad.EaseInOut, "Y", (-m_gate.Height).ToString());
        m_gateClosed = false;
        RoomCompleted = true;
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        else if (IsReversed && m_isPlayingGame && Player.X > Bounds.Right - 10) Player.X = Bounds.Right - 10;
    ...
    private void HandleInput()
    {
            ...
            Player.SpellCastDelay <= 0f && m_gateClosed)
        {
            m_axesThrown++;
            Player.CurrentMana = Player.MaxMana;
            if (m_axesThrown <= m_numTries) m_axeIcons.GetChildAt(m_numTries - m_axesThrown).Visible = false;

            if (m_axesThrown > m_numTries) Game.PlayerStats.Spell = 0;
        }
    }
