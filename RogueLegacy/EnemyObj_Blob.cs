...
public class EnemyObj_Blob : EnemyObj
{
    ...
    private readonly LogicBlock m_generalBossCooldownLB         = new();
    ...
    private          Vector2    BlobSizeChange                  = new(0.4f, 0.4f);
    ...
    protected override void InitializeEV()
    {
        ...
        if (Difficulty == EnemyDifficulty.MiniBoss) m_resetSpriteName = "EnemyBlobBossIdle_Character";
    }
    ...
    protected override void RunMinibossLogic()
    {
        if (m_isTouchingGround)
        {
            switch (State)
            {
                ...
                case 2:
                case 3:
                    if (!IsNeo) RunLogicBlock(true, m_generalMiniBossLB, 45, 0, 0, 55);
            ...
            if (CurrentHealth <= 0)
            {
                ...
                if (!IsKilled && NumHits > 0)
                {
                    if (!IsNeo)
                    {
                        ...
                        if (Difficulty == EnemyDifficulty.MiniBoss) CreateWizard();
                    }
                    else
                    {
                        if (m_flipTween != null && m_flipTween.TweenedObject == this && m_flipTween.Active)
