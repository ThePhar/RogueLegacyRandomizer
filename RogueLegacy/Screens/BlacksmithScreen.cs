...
//  Last Modified 2023-10-25 11:03 PM
// 
...
public class BlacksmithScreen : Screen
{
    ...
    {
        _blacksmithUI = new("BlacksmithUI_Character")
        {
            Position = new(660f, 360f)
        };
        ...
        foreach (var current in _masterIconArray)
        {
            ...
            {
                current[j] = new("BlacksmithUI_QuestionMarkIcon_Character")
                {
        ...
        _cancelText = new(Game.JunicodeFont)
        {
            ...
            FontSize = 12f,
            Position = new(_confirmText.X, _confirmText.Y + 40f),
            ForceDraw = true
        };
        _navigationText = new(Game.JunicodeFont)
        {
            Text = "to move map",
            FontSize = 12f,
            Position = new(_confirmText.X, _confirmText.Y + 80f),
            ForceDraw = true
        };
        _newIconList = new();
        for (var k = 0; k < 25; k++)
    ...
    {
        _textInfoTitleContainer = new();
        _textInfoStatContainer = new();
        _textInfoStatModContainer = new();
        string[] array =
        {
            "Health",
            "Mana",
        ...
        _equipmentTitleText = new(Game.JunicodeFont)
        {
            ForceDraw = true,
            FontSize = 12f,
            DropShadow = new(2f, 2f),
            TextureColor = new(237, 202, 138)
        };
        _textInfoTitleContainer.Position = new(_blacksmithUI.X + 205f, _blacksmithUI.Y - _blacksmithUI.Height / 2 + 45f);
        _textInfoStatContainer.Position = new(_textInfoTitleContainer.X + 15f, _textInfoTitleContainer.Y);
        _textInfoStatModContainer.Position = new(_textInfoStatContainer.X + 75f, _textInfoStatContainer.Y);
        _addPropertiesTitleText.Position = new(_blacksmithUI.X + 140f, _textInfoStatModContainer.Bounds.Bottom + 5);
        _addPropertiesText.Position = new(_addPropertiesTitleText.X, _addPropertiesTitleText.Bounds.Bottom);
        _unlockCostContainer.Position = new(_blacksmithUI.X + 114f, 485f);
        _equipmentTitleText.Position = new(_blacksmithUI.X + 140f, _textInfoTitleContainer.Y - 45f);
    ...
    private void DisplayCategory(int equipmentType)
    {
        ...
        if (_activeIconArray != null)
        {
            for (var i = 0; i < 15; i++)
            {
                ...
                Tween.To(_activeIconArray[i], duration, Back.EaseIn, "delay", $"{num}", "ScaleX", "0", "ScaleY", "0");
            }
        }

        _activeIconArray = _masterIconArray[equipmentType];
        if (RandomizerData.ShuffleBlacksmith)
        {
        ...
        for (var j = 0; j < 15; j++)
        {
            Tween.To(_activeIconArray[j], duration, Back.EaseOut, "delay", $"{num}", "ScaleX", "1", "ScaleY", "1");
        }
    ...
    private void UpdateIconStates()
    {
            ...
            if (RandomizerData.ShuffleBlacksmith)
            {
                switch ((EquipmentCategory) i)
                {
                    case EquipmentCategory.Sword:
                        _blacksmithUI.GetChildAt(i + 6)
                            .ChangeSprite(SkillSystem.GetSkill(SkillType.BlacksmithSword).CurrentLevel == 0
                                ? "BlacksmithUI_QuestionMarkIcon_Sprite"
                                : "BlacksmithUI_SwordCategoryIcon_Sprite");
                        break;
                    case EquipmentCategory.Helm:
                        _blacksmithUI.GetChildAt(i + 6)
                            .ChangeSprite(SkillSystem.GetSkill(SkillType.BlacksmithHelm).CurrentLevel == 0
                                ? "BlacksmithUI_QuestionMarkIcon_Sprite"
                                : "BlacksmithUI_HelmCategoryIcon_Sprite");
                        break;
                    case EquipmentCategory.Chest:
                        _blacksmithUI.GetChildAt(i + 6)
                            .ChangeSprite(SkillSystem.GetSkill(SkillType.BlacksmithChest).CurrentLevel == 0
                                ? "BlacksmithUI_QuestionMarkIcon_Sprite"
                                : "BlacksmithUI_ChestCategoryIcon_Sprite");
                        break;
                    case EquipmentCategory.Limbs:
                        _blacksmithUI.GetChildAt(i + 6)
                            .ChangeSprite(SkillSystem.GetSkill(SkillType.BlacksmithLimbs).CurrentLevel == 0
                                ? "BlacksmithUI_QuestionMarkIcon_Sprite"
                                : "BlacksmithUI_LimbsCategoryIcon_Sprite");
                        break;
                    case EquipmentCategory.Cape:
                        _blacksmithUI.GetChildAt(i + 6)
                            .ChangeSprite(SkillSystem.GetSkill(SkillType.BlacksmithCape).CurrentLevel == 0
                                ? "BlacksmithUI_QuestionMarkIcon_Sprite"
        ...
        for (var j = 0; j < Game.PlayerStats.GetBlueprintArray[i].Length; j++)
        {
            ...
            {
                _masterIconArray[i][j].ChangeSprite("BlacksmithUI_" + (EquipmentCategory) i + (j % 5 + 1) + "Icon_Character");
    ...
    private void CategorySelectionInput()
    {
        ...
        if (currentCategoryIndex != _currentCategoryIndex)
        {
            ...
            for (var i = 1; i < 6; i++)
            {
                if (i == 1)
                {
                    _blacksmithUI.GetChildAt(i).Scale = new(1f, 1f);
                }
                else
                {
                    _blacksmithUI.GetChildAt(i).Scale = Vector2.Zero;
            ...
            if (_currentCategoryIndex != 6)
            {
                _blacksmithUI.GetChildAt(_currentCategoryIndex - 5).Scale = new(1f, 1f);
            }
            else
            {
        ...
        {
            _lockControls = true;
            _equipmentDescriptionText.Text = "";
    ...
    private void UpdateIconSelectionText()
    {
        ...
        if (_inCategoryMenu)
        {
            ...
            {
                EquipmentCategory.Sword + 6 => SkillSystem.GetSkill(SkillType.BlacksmithSword).CurrentLevel == 0
                    ? "Sword slot needed"
                    : "",
                EquipmentCategory.Helm + 6 => SkillSystem.GetSkill(SkillType.BlacksmithHelm).CurrentLevel == 0
                    ? "Helm slot needed"
                    : "",
                EquipmentCategory.Chest + 6 => SkillSystem.GetSkill(SkillType.BlacksmithChest).CurrentLevel == 0
                    ? "Chest slot needed"
                    : "",
                EquipmentCategory.Limbs + 6 => SkillSystem.GetSkill(SkillType.BlacksmithLimbs).CurrentLevel == 0
                    ? "Limbs slot needed"
                    : "",
                EquipmentCategory.Cape + 6 => SkillSystem.GetSkill(SkillType.BlacksmithCape).CurrentLevel == 0
                    ? "Cape slot needed"
                    : "",
