...
namespace RogueCastle
{
	public class PlayerObj : CharacterObj, IDealsDamageObj
	{
		...
		private Rectangle GroundCollisionRect
		{
			get
			{
				return new Rectangle(Bounds.X, Bounds.Y, Bounds.Width, Bounds.Height + StepUp);
			}
		}
		...
		public int InvulnDamage
		{
			get
			{
				return (int)(RandomDamage * (1f + SkillSystem.GetSkill(SkillType.Invuln_Attack_Up).ModifierAmount) + SkillSystem.GetSkill(SkillType.Attack_Up).ModifierAmount + SkillSystem.GetSkill(SkillType.Damage_Up_Final).ModifierAmount);
			}
		}
		...
		public int RandomDamage
		{
			get
			{
				return CDGMath.RandomInt(MinDamage + GetEquipmentDamage(), MaxDamage + GetEquipmentDamage()) + Game.PlayerStats.BonusStrength + DamageGainPerLevel * Game.PlayerStats.CurrentLevel;
			}
		}
		...
		public int MaxWeight
		{
			get
			{
				return (int)(BaseWeight + SkillSystem.GetSkill(SkillType.Equip_Up).ModifierAmount + SkillSystem.GetSkill(SkillType.Equip_Up_Final).ModifierAmount) + Game.PlayerStats.BonusWeight * 5;
			}
		}
		public bool CanFly
		{
			get
			{
				return Game.PlayerStats.Class == 16 || TotalFlightTime > 0f;
			}
		}
		...
		public byte AttacksNeededForMana
		{
			get
			{
				return m_attacksNeededForMana;
			}
			set
			{
				m_attacksNeededForMana = value;
			}
		}
		...
		public float BlockManaDrain
		{
			get
			{
				return m_blockManaDrain - (int)GetEquipmentSecondaryAttrib(8) - (int)SkillSystem.GetSkill(SkillType.Block).ModifierAmount;
			}
			set
			{
				m_blockManaDrain = value;
			}
		}
		public float TotalStatDropChance
		{
			get
			{
				return StatDropIncrease + BaseStatDropChance;
			}
		}
		...
		public float TotalXPBonus
		{
			get
			{
				return SkillSystem.GetSkill(SkillType.XP_Gain_Up).ModifierAmount + GetEquipmentSecondaryAttrib(5);
			}
		}
		...
		public int TotalVampBonus
		{
			get
			{
				return (int)((Game.PlayerStats.GetNumberOfEquippedRunes(2) * 2 + (int)GetEquipmentSecondaryAttrib(7) * 2 + Game.PlayerStats.GetNumberOfEquippedRunes(10)) * (1f + Game.PlayerStats.TimesCastleBeaten * 0.5f));
			}
		}
		public int TotalAirDashes
		{
			get
			{
				return Game.PlayerStats.GetNumberOfEquippedRunes(1) + (int)GetEquipmentSecondaryAttrib(11);
			}
		}
		public int TotalDoubleJumps
		{
			get
			{
				return Game.PlayerStats.GetNumberOfEquippedRunes(0) + (int)GetEquipmentSecondaryAttrib(9);
			}
		}
		...
		public float TotalArmor
		{
			get
			{
				return SkillSystem.GetSkill(SkillType.Armor_Up).ModifierAmount + Game.PlayerStats.BonusDefense * 2 + GetEquipmentArmor();
			}
		}
		public float TotalDamageReduc
		{
			get
			{
				return TotalArmor / (ArmorReductionMod + TotalArmor);
			}
		}
		...
		public float TotalDamageReturn
		{
			get
			{
				return GetEquipmentSecondaryAttrib(4) + Game.PlayerStats.GetNumberOfEquippedRunes(5) * 0.5f;
			}
		}
		...
		public override void ChangeSprite(string spriteName)
		{
			...
			if (State != 8)
			{
				...
				string text2 = (_objectList[4] as IAnimateableObj).SpriteName;
				...
				string text3 = (_objectList[9] as IAnimateableObj).SpriteName;
				...
				string text4 = (_objectList[3] as IAnimateableObj).SpriteName;
				...
				{
					if (State == 1 && (m_currentLogicSet != m_standingAttack3LogicSet || (m_currentLogicSet == m_standingAttack3LogicSet && !m_currentLogicSet.IsActive)))
					{
						_objectList[6].ChangeSprite("PlayerWalkingArmsSpellSword_Sprite");
					}
					else if (State == 2 && (m_currentLogicSet != m_standingAttack3LogicSet || (m_currentLogicSet == m_standingAttack3LogicSet && !m_currentLogicSet.IsActive)) && !IsAirAttacking && !IsKilled)
					{
						if (AccelerationY < 0f)
						{
		...
		public override void HandleInput()
		{
			...
			if (!m_lockControls && !IsKilled && State != 3)
			{
				InputControls();
			}
			if (Game.PlayerStats.Class == 17 && !(AttachedLevel.CurrentRoom is CarnivalShoot1BonusRoom) && !(AttachedLevel.CurrentRoom is CarnivalShoot2BonusRoom) && !IsKilled && State == 4 && Game.GlobalInput.JustPressed(24) && m_spellCastDelay <= 0f && CurrentMana >= 30f)
			{
		...
		private void InputControls()
		{
			...
			{
				if ((Game.GlobalInput.JustPressed(18) || Game.GlobalInput.JustPressed(19)) && CanAirAttackDownward && Game.GameConfig.QuickDrop && State == 2 && m_dropThroughGroundTimer <= 0f)
				{
			...
			if (m_dashCooldownCounter <= 0 && (m_isTouchingGround || (!m_isTouchingGround && m_airDashCount < TotalAirDashes)) && State != 6 && State != 8 && CanAirDash)
			{
			...
			else if ((Game.GlobalInput.JustPressed(10) || Game.GlobalInput.JustPressed(11)) && !m_isTouchingGround && !flag && m_doubleJumpCount >= TotalDoubleJumps && m_dropThroughGroundTimer <= 0f && CanFly && m_flightCounter > 0f && State != 7 && State != 9 && State != 6 && State != 8)
			{
				AccelerationY = 0f;
				State = 7;
				DisableGravity = true;
		...
		public override void Update(GameTime gameTime)
		{
			...
			{
				if ((Game.PlayerStats.Traits.Y == 22f || Game.PlayerStats.Traits.X == 22f) && CurrentSpeed == 0f && m_ambilevousTimer > 0f)
				{
		...
		private void UpdateAnimationState()
		{
			...
			case 4:
				...
				else if (HeadingX < 0f && Flip == SpriteEffects.FlipHorizontally && SpriteName != "PlayerFrontDash_Character")
				{
				...
				else if (HeadingX > 0f && Flip == SpriteEffects.FlipHorizontally && SpriteName != "PlayerDash_Character")
				{
					ChangeSprite("PlayerDash_Character");
				}
				if (!IsAnimating)
		...
		private void CheckGroundCollision()
		{
			...
			Rectangle rectangle = new Rectangle(TerrainBounds.Left, TerrainBounds.Bottom - 78, TerrainBounds.Width, 88);
			foreach (IPhysicsObj current in PhysicsMngr.ObjectList)
			{
				if (((Game.PlayerStats.Traits.X != 33f && Game.PlayerStats.Traits.Y != 33f) || !(current is PhysicsObj) || current is HazardObj) && current != this && current.Visible && current.IsCollidable && (current.CollidesTop || current.CollidesLeft || current.CollidesRight) && current.HasTerrainHitBox && (current.CollisionTypeTag == 1 || current.CollisionTypeTag == 10 || current.CollisionTypeTag == 5 || current.CollisionTypeTag == 4) && (!current.CollidesTop || current.CollidesBottom || (State != 7 && State != 9)) && (!current.CollidesTop || !current.CollidesBottom || current.CollidesLeft || current.CollidesRight))
				{
					...
					if (current.Rotation == 0f || hazardObj != null)
					{
						...
						Rectangle rectA = new Rectangle(TerrainBounds.X, TerrainBounds.Y, TerrainBounds.Width, TerrainBounds.Height);
						...
						Vector2 vector3 = CollisionMath.CalculateMTD(TerrainBounds, rectangle2);
						if (vector3.X > 0f)
						{
						...
						if (current.CollidesBottom || Math.Abs(rectangle2.Top - TerrainBounds.Bottom) <= 20 || AccelerationY >= 1100f)
						{
							int num3 = (int)Math.Abs(intersectionDepth.X);
							int num4 = (int)Math.Abs(intersectionDepth.Y);
							if ((num3 <= 1 || num3 >= num4) && ((!m_isJumping && AccelerationY >= 0f) || (m_isJumping && AccelerationY >= 0f)) && (vector.Y < 0f || (vector.Y == 0f && vector.X != 0f && rectangle2.Top > TerrainBounds.Top && current.Y > Y)))
							{
								int num5 = Math.Abs(rectangle2.Top - TerrainBounds.Bottom);
								if (num5 < num)
								{
									physicsObj = current;
									num = num5;
					...
					else if (((!m_isJumping && AccelerationY >= 0f) || (m_isJumping && AccelerationY >= 0f)) && !(current is HazardObj))
					{
						Vector2 vector4 = CollisionMath.RotatedRectIntersectsMTD(rectangle, Rotation, Vector2.Zero, current.TerrainBounds, current.Rotation, Vector2.Zero);
						if (vector4.Y < 0f)
						{
							float y = vector4.Y;
							if (y < num2)
			...
			if (m_isTouchingGround)
			{
				if (State == 2 || State == 7 || State == 3)
				{
					if (Game.PlayerStats.Traits.X == 6f || Game.PlayerStats.Traits.Y == 6f)
					{
						...
						if (AccelerationY > 1400f)
						{
							...
							AttachedLevel.ImpactEffectPool.DisplayDustEffect(new Vector2(TerrainBounds.Left, Bounds.Bottom));
							AttachedLevel.ImpactEffectPool.DisplayDustEffect(new Vector2(TerrainBounds.X, Bounds.Bottom));
							AttachedLevel.ImpactEffectPool.DisplayDustEffect(new Vector2(TerrainBounds.Right, Bounds.Bottom));
						}
					}
					if (Game.PlayerStats.Traits.X == 7f || Game.PlayerStats.Traits.Y == 7f)
					{
				...
				if (State != 6 && State != 8 && State != 4 && physicsObj != null && !ControlsLocked && (((Game.GlobalInput.Pressed(18) || Game.GlobalInput.Pressed(19)) && (Game.GlobalInput.JustPressed(10) || Game.GlobalInput.JustPressed(11))) || (Game.GameConfig.QuickDrop && (Game.GlobalInput.JustPressed(18) || Game.GlobalInput.JustPressed(19)))) && !physicsObj.CollidesBottom && State != 8)
				{
					AccelerationY = 0f;
					Y += 15f;
					m_isTouchingGround = false;
		...
		public override void CollisionResponse(CollisionBox thisBox, CollisionBox otherBox, int collisionResponseType)
		{
			...
			if (teleporterObj != null && !ControlsLocked && IsTouchingGround && (Game.GlobalInput.JustPressed(16) || Game.GlobalInput.JustPressed(17)))
			{
				StopAllSpells();
				LockControls();
			...
			if (collisionResponseType == 1 && (physicsObj.CollisionTypeTag == 1 || physicsObj.CollisionTypeTag == 5 || physicsObj.CollisionTypeTag == 4 || physicsObj.CollisionTypeTag == 10))
			{
				...
				if (m_isTouchingGround && !physicsObj.CollidesBottom && physicsObj.CollidesTop && physicsObj.TerrainBounds.Top < TerrainBounds.Bottom - 10)
				{
					flag = false;
				}
			...
			if (thisBox.Type == 2 && otherBox.Type == 1 && (physicsObj.CollisionTypeTag == 3 || physicsObj.CollisionTypeTag == 4 || physicsObj.CollisionTypeTag == 10) && State != 3 && m_invincibleCounter <= 0)
			{
				...
				if (!LevelEV.ENABLE_PLAYER_DEBUG)
				{
					if (State == 6 && (CurrentMana > 0f || m_blockInvincibleCounter > 0f) && (projectileObj == null || (projectileObj != null && projectileObj.Spell != 8 && projectileObj.Spell != 12)))
					{
						if (CanBeKnockedBack)
						{
		...
		public void HitPlayer(GameObj obj)
		{
			...
			if (flag && (!ForceInvincible || (ForceInvincible && obj is HazardObj)))
			{
				...
				if (!m_levelScreen.IsDisposed)
				{
					...
					{
						m_levelScreen.TextManager.DisplayNumberText(num * 100 + CDGMath.RandomInt(1, 99), Color.Red, new Vector2(X, Bounds.Top));
					}
					else
					{
				...
				if (Game.PlayerStats.SpecialItem == 2)
				{
					...
					if (num7 > 0 && AttachedLevel.ItemDropManager.AvailableItems > num7)
					{
						...
						if (num9 > 0)
						{
							AttachedLevel.TextManager.DisplayNumberStringText(-num9, "gold", Color.Yellow, new Vector2(X, Bounds.Top));
						}
					}
				}
				if (Game.PlayerStats.IsFemale)
		...
		public void ConvertHPtoMP()
		{
			if (MaxHealth > 1)
			{
				...
				if (MaxMana + num + num2 < num3)
				{
					...
					m_levelScreen.TextManager.DisplayNumberStringText(num + num2, "max mp", Color.RoyalBlue, new Vector2(X, Bounds.Top - 30));
					m_levelScreen.TextManager.DisplayNumberStringText(-(num + num2), "max hp", Color.Red, new Vector2(X, Bounds.Top - 60));
					return;
				}
				SoundManager.PlaySound("Error_Spell");
				m_levelScreen.TextManager.DisplayStringText("Max MP Converted. Need higher level.", Color.RoyalBlue, new Vector2(X, Bounds.Top - 30));
			}
		}
		...
		public TerrainObj CalculateClosestWall(out int dist)
		{
			...
			foreach (TerrainObj current in currentRoom.TerrainObjList)
			{
				if (current.CollidesBottom || current.CollidesLeft || current.CollidesRight)
				{
					...
					if (Flip == SpriteEffects.None)
					{
						if (current.X > X && current.Bounds.Top + 5 < TerrainBounds.Bottom && current.Bounds.Bottom > TerrainBounds.Top)
						{
					...
					else if (current.X < X && current.Bounds.Top + 5 < TerrainBounds.Bottom && current.Bounds.Bottom > TerrainBounds.Top)
					{
						if (current.Rotation < 0f)
						{
		...
		{
			SpriteObj spriteObj = _objectList[10] as SpriteObj;
			SpriteObj spriteObj2 = _objectList[11] as SpriteObj;
