...
namespace RogueCastle
{
	public class EnemyObj_LastBoss : EnemyObj
	{
		...
		public override void Update(GameTime gameTime)
		{
			...
			if (!m_inSecondForm)
			{
				if (!m_isTouchingGround && m_currentActiveLB != null && SpriteName != "PlayerAttacking3_Character" && !m_isDashing && SpriteName != "PlayerLevelUp_Character")
				{
			...
			if ((!m_firstFormDying && !m_inSecondForm) || (m_firstFormDying && m_inSecondForm) || (IsNeo && CurrentHealth > 0))
			{
				base.Update(gameTime);
			}
			if (!m_inSecondForm && CurrentHealth <= 0 && m_target.CurrentHealth > 0 && IsNeo && IsTouchingGround && !m_firstFormDying)
			{
				KillPlayerNeo();
				m_firstFormDying = true;
			}
		...
		public void MovePlayerTo()
		{
			...
			int num = 250;
			Vector2 zero = Vector2.Zero;
			if ((m_target.X < X && X > m_levelScreen.CurrentRoom.X + 500f) || X > m_levelScreen.CurrentRoom.Bounds.Right - 500)
			{
