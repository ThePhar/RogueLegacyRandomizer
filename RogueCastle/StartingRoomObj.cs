...
namespace RogueCastle
{
	public class StartingRoomObj : RoomObj
	{
		...
		private bool TollCollectorAvailable
		{
			get
			{
				return Game.PlayerStats.TimesDead > 0 && m_tollCollector.Visible;
			}
		}
		public StartingRoomObj()
		{
			...
			m_blacksmithBoard.Position = new Vector2(m_blacksmith.X - m_blacksmithBoard.Width / 2 - 35f, m_blacksmith.Bounds.Bottom - m_blacksmithBoard.Height - 1);
			...
			m_tent.Position = new Vector2(m_enchantress.X - m_tent.Width / 2 + 5f, m_enchantress.Bounds.Bottom - m_tent.Height);
			...
			for (int i = 0; i < num; i++)
			{
				RaindropObj item = new RaindropObj(new Vector2(CDGMath.RandomInt(-100, 2540), CDGMath.RandomInt(-400, 720)));
				m_rainFG.Add(item);
			}
		}
		...
		public override void Update(GameTime gameTime)
		{
			...
			m_tollCollectorIcon.Position = new Vector2(m_tollCollector.X - m_tollCollector.Width / 2 - 10f, m_tollCollector.Y - m_tollCollectorIcon.Height - m_tollCollector.Height / 2 + (float)Math.Sin(totalGameTime * 20f) * 2f);
			m_blacksmithNewIcon.Visible = false;
			if (SmithyAvailable)
			{
		...
		public void UpdateShake()
		{
			...
			{
				Player.AttachedLevel.Camera.X = m_shakeStartingPos.X + CDGMath.RandomPlusMinus() * (CDGMath.RandomFloat(0f, 1f) * m_screenShakeMagnitude);
			}
			if (m_verticalShake)
			{
