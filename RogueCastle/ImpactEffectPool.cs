...
namespace RogueCastle
{
	public class ImpactEffectPool : IDisposableObj
	{
		...
		public ImpactEffectPool(int poolSize)
		{
			...
			this.m_resourcePool = new DS2DPool<SpriteObj>();
		}
		public void Initialize()
		{
			for (int i = 0; i < this.m_poolSize; i++)
			{
				...
				this.m_resourcePool.AddToPool(spriteObj);
			}
		}
		public SpriteObj DisplayEffect(Vector2 position, string spriteName)
		{
			SpriteObj spriteObj = this.m_resourcePool.CheckOut();
		...
		public void DisplayTeleportEffect(Vector2 position)
		{
			...
			SpriteObj spriteObj2 = this.m_resourcePool.CheckOut();
		...
		{
			float num = duration / (float)numClouds;
			float num2 = 0f;
		...
		{
			float num = 1320f / (float)numEntities;
			for (int i = 0; i < numEntities; i++)
			{
				...
				spriteObj.Position = new Vector2(num * (float)i, 720f);
				spriteObj.PlayAnimation(true);
				spriteObj.Opacity = 0f;
		...
		public void DestroyFireballBoss(Vector2 pos)
		{
			...
			for (int i = 0; i < 15; i++)
			{
				...
				{
					"X",
					(vector.X * (float)num4).ToString(),
					"Y",
					(vector.Y * (float)num4).ToString()
				});
				Tween.To(spriteObj, 0.5f, new Easing(Tween.EaseNone), new string[]
				{
					"delay",
		...
		public void SkillTreeDustEffect(Vector2 pos, bool horizontal, float length)
		{
			...
			float scaleFactor = length / (float)num;
		...
		{
			int num = 15;
			float num2 = (float)sprite.Width / (float)num;
			...
			num /= 2;
			num3 = (float)(sprite.Bounds.Left + 50);
			num2 = (float)(sprite.Width - 50) / (float)num;
			for (int j = 0; j < num; j++)
			{
				SpriteObj spriteObj2 = this.m_resourcePool.CheckOut();
			...
			num /= 2;
			num3 = (float)(sprite.Bounds.Left + 100);
			num2 = (float)(sprite.Width - 100) / (float)num;
			for (int k = 0; k < num; k++)
			{
				SpriteObj spriteObj3 = this.m_resourcePool.CheckOut();
		...
		public void PauseAllAnimations()
		{
			...
			foreach (SpriteObj current in this.m_resourcePool.ActiveObjsList)
			{
		...
		public void Draw(Camera2D camera)
		{
			...
			{
				if (!this.m_resourcePool.ActiveObjsList[i].IsAnimating && !this.m_isPaused)
				{
					this.DestroyEffect(this.m_resourcePool.ActiveObjsList[i]);
					i--;
				}
				else
				{
					this.m_resourcePool.ActiveObjsList[i].Draw(camera);
				}
			}
		}
		public void Dispose()
