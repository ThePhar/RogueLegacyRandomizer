						...
						if ((doorPosition = current.DoorPosition) != null)
						{
							if (!(doorPosition == "Left"))
							{
								if (!(doorPosition == "Right"))
								{
									...
									{
										spriteObj2.Rotation = -90f;
										spriteObj2.Position = new Vector2(current.X / m_spriteScale.X, (current.Y + current.Height) / m_spriteScale.Y + 2f);
										flag = true;
									}
								}
								else
								{
									spriteObj2.Position = new Vector2(room.Bounds.Right / m_spriteScale.X - 5f, current.Y / m_spriteScale.Y - 2f);
									flag = true;
								}
							}
							else
							{
								spriteObj2.Position = new Vector2(room.Bounds.Left / m_spriteScale.X - spriteObj2.Width + 2f, current.Y / m_spriteScale.Y - 2f);
								flag = true;
							}
						}
						if (flag)
