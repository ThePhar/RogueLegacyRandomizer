	...
	internal class LevelBuilder2
	{
		...
		public static void StoreSpecialRoom(RoomObj room, GameTypes.LevelType levelType, bool storeDebug = false)
		{
			...
			case "Linker":
			{
				...
				teleporterObj.Position = new Vector2(roomObj.X + roomObj.Width / 2f - (teleporterObj.Bounds.Right - teleporterObj.AnchorX), roomObj.Y + roomObj.Height - 60f);
			...
			case "EntranceBoss":
			{
				...
				teleporterObj2.Position = new Vector2(roomObj2.X + roomObj2.Width / 2f - (teleporterObj2.Bounds.Right - teleporterObj2.AnchorX), roomObj2.Y + roomObj2.Height - 60f);
		...
		public static List<RoomObj> CreateArea(int areaSize, AreaStruct areaInfo, List<RoomObj> roomsToCheckCollisionsList, RoomObj startingRoom, bool firstRoom)
		{
			...
			if (firstRoom)
			{
				...
				list5.Add(roomObj);
				i--;
				MoveRoom(roomObj, new Vector2(startingRoom.X + startingRoom.Width, startingRoom.Bounds.Bottom - roomObj.Height));
					...
					if (num >= CDGMath.RandomInt(50, 100) && !flag)
					{
						...
						using (List<DoorObj>.Enumerator enumerator2 = roomObj2.DoorList.GetEnumerator())
						{
							while (enumerator2.MoveNext())
							{
								...
								if (current2.DoorPosition == oppositeDoorPosition && !CheckForRoomCollision(doorObj2, list5, current2) && !list8.Contains(current2))
								{
									flag = true;
									list8.Add(current2);
									break;
						...
						if (!(doorPosition2 == "Left"))
						{
							if (!(doorPosition2 == "Right"))
							{
								if (!(doorPosition2 == "Top"))
								{
									...
									{
										zero = new Vector2(doorObj2.X - (doorObj3.X - doorObj3.Room.X), doorObj2.Y + doorObj2.Height);
									}
								}
								else
								{
									zero = new Vector2(doorObj2.X - (doorObj3.X - doorObj3.Room.X), doorObj2.Y - doorObj3.Room.Height);
								}
							}
							else
							{
								zero = new Vector2(doorObj2.X + doorObj2.Width, doorObj2.Y - (doorObj3.Y - doorObj3.Room.Y));
							}
						}
						else
						{
							zero = new Vector2(doorObj2.X - doorObj3.Room.Width, doorObj2.Y - (doorObj3.Y - doorObj3.Room.Y));
						}
					}
		...
		private static List<DoorObj> FindSuitableDoors(DoorObj doorToCheck, int roomWidth, int roomHeight, List<RoomObj> roomList, GameTypes.LevelType levelType, bool findRoomsWithMoreDoors)
		{
				...
				for (int j = 1; j <= roomHeight; j++)
				{
					...
					foreach (RoomObj current in roomList2)
					{
						...
						if (flag)
						{
							...
							{
								if (current2.DoorPosition == oppositeDoorPosition && !CheckForRoomCollision(doorToCheck, roomList, current2) && !list.Contains(current2))
								{
									list.Add(current2);
								}
							}
			...
			foreach (RoomObj current3 in sequencedDLCRoomList)
			{
				...
				if (flag2)
				{
					...
					{
						if (current4.DoorPosition == oppositeDoorPosition && !CheckForRoomCollision(doorToCheck, roomList, current4) && !list.Contains(current4))
						{
							list.Add(current4);
						}
					}
		...
		private static List<DoorObj> FindSuitableDoors(DoorObj doorToCheck, List<RoomObj> roomList, List<RoomObj> roomCollisionList)
		{
			...
			foreach (RoomObj current in roomList)
			{
				if (Game.PlayerStats.DiaryEntry < 24 || !(current.Name == "Bonus") || !(current.Tag == 6.ToString()))
				{
					foreach (DoorObj current2 in current.DoorList)
					{
						if (current2.DoorPosition == oppositeDoorPosition && !CheckForRoomCollision(doorToCheck, roomCollisionList, current2) && !list.Contains(current2))
						{
							list.Add(current2);
						}
					}
					...
					if ((doorPosition = current2.DoorPosition) != null)
					{
						if (!(doorPosition == "Left") && !(doorPosition == "Right"))
						{
							if (doorPosition == "Top" || doorPosition == "Bottom")
							{
								int num = 0;
								BorderObj borderObj = new BorderObj();
								borderObj.Position = new Vector2(current2.Room.X + (current2.X - current2.Room.X) + current2.Width, current2.Room.Y + (current2.Y - current2.Room.Y) + num);
								...
								borderObj2.Position = new Vector2(current2.Room.X + (current2.X - current2.Room.X) - 60f, current2.Room.Y + (current2.Y - current2.Room.Y) + num);
						...
						else
						{
							...
							borderObj4.Position = new Vector2(current2.Room.X + (current2.X - current2.Room.X), current2.Room.Y + (current2.Y - current2.Room.Y) + current2.Height);
				...
				if (!(doorPosition == "Left"))
				{
					if (!(doorPosition == "Right"))
					{
						if (!(doorPosition == "Top"))
						{
							if (doorPosition == "Bottom")
							{
								zero = new Vector2(needsLinking.X - (doorObj.X - doorObj.Room.X), needsLinking.Y + needsLinking.Height);
							}
						}
						else
						{
							zero = new Vector2(needsLinking.X - (doorObj.X - doorObj.Room.X), needsLinking.Y - doorObj.Room.Height);
						}
					}
					else
					{
						zero = new Vector2(needsLinking.X + needsLinking.Width, needsLinking.Y - (doorObj.Y - doorObj.Room.Y));
					}
				}
				else
				{
					zero = new Vector2(needsLinking.X - doorObj.Room.Width, needsLinking.Y - (doorObj.Y - doorObj.Room.Y));
				}
			}
		...
		private static void RemoveFromListHelper<T>(List<T> list)
		{
			for (int i = 0; i < list.Count; i++)
			{
				...
				if (name != null)
				{
					if ((!hasTopLeftDoor && name.IndexOf("TopLeft") != -1 && name.IndexOf("!TopLeft") == -1) || (hasTopLeftDoor && name.IndexOf("!TopLeft") != -1) || (!hasTopRightDoor && name.IndexOf("TopRight") != -1 && name.IndexOf("!TopRight") == -1) || (hasTopRightDoor && name.IndexOf("!TopRight") != -1) || (!hasTopDoor && name.IndexOf("Top") != -1 && name.IndexOf("!Top") == -1 && name.Length == 3) || (hasTopDoor && name.IndexOf("!Top") != -1 && name.Length == 4))
					{
						list.Remove(list[i]);
						i--;
					}
					if ((!hasBottomLeftDoor && name.IndexOf("BottomLeft") != -1 && name.IndexOf("!BottomLeft") == -1) || (hasBottomLeftDoor && name.IndexOf("!BottomLeft") != -1) || (!hasBottomRightDoor && name.IndexOf("BottomRight") != -1 && name.IndexOf("!BottomRight") == -1) || (hasBottomRightDoor && name.IndexOf("!BottomRight") != -1) || (!hasBottomDoor && name.IndexOf("Bottom") != -1 && name.IndexOf("!Bottom") == -1 && name.Length == 6) || (hasBottomDoor && name.IndexOf("!Bottom") != -1 && name.Length == 7))
					{
						list.Remove(list[i]);
						i--;
					}
					if ((!hasLeftTopDoor && name.IndexOf("LeftTop") != -1 && name.IndexOf("!LeftTop") == -1) || (hasLeftTopDoor && name.IndexOf("!LeftTop") != -1) || (!hasLeftBottomDoor && name.IndexOf("LeftBottom") != -1 && name.IndexOf("!LeftBottom") == -1) || (hasLeftBottomDoor && name.IndexOf("!LeftBottom") != -1) || (!hasLeftDoor && name.IndexOf("Left") != -1 && name.IndexOf("!Left") == -1 && name.Length == 4) || (hasLeftDoor && name.IndexOf("!Left") != -1 && name.Length == 5))
					{
						list.Remove(list[i]);
						i--;
					}
					if ((!hasRightTopDoor && name.IndexOf("RightTop") != -1 && name.IndexOf("!RightTop") == -1) || (hasRightTopDoor && name.IndexOf("!RightTop") != -1) || (!hasRightBottomDoor && name.IndexOf("RightBottom") != -1 && name.IndexOf("!RightBottom") == -1) || (hasRightBottomDoor && name.IndexOf("!RightBottom") != -1) || (!hasRightDoor && name.IndexOf("Right") != -1 && name.IndexOf("!Right") == -1 && name.Length == 5) || (hasRightDoor && name.IndexOf("!Right") != -1 && name.Length == 6))
					{
						list.Remove(list[i]);
						i--;
					}
		...
		public static void OverrideProceduralEnemies(ProceduralLevelScreen level, byte[] enemyTypeData, byte[] enemyDifficultyData)
		{
			...
			foreach (RoomObj current in level.RoomList)
			{
				for (int i = 0; i < current.EnemyList.Count; i++)
				{
					...
					{
						EnemyObj enemyObj2 = EnemyBuilder.BuildEnemy(enemyTypeData[num], level.Player, null, level, GameTypes.EnemyDifficulty.BASIC, true);
						enemyObj2.IsProcedural = true;
		...
		public static ProceduralLevelScreen CreateTutorialRoom()
		{
			...
			MoveRoom(tutorialRoomObj, new Vector2(introRoomObj.Width, -(float)tutorialRoomObj.Height + introRoomObj.Height));
			...
			while (true)
			{
					...
					while (true)
					{
						...
						while (array[j] == null || array[j].Count < num2 || !flag)
						{
							...
							if (j == 0)
							{
								...
								{
									array[j] = CreateArea(num2, areaInfo, list, StartingRoom.Clone() as StartingRoomObj, true);
								}
								else
								{
									array[j] = CreateArea(num2, areaInfo, list, startingRoom.Clone() as StartingRoomObj, true);
						...
						if (FindFurthestDoor(array[j], "Right", "Right", false, false) != null && FindFurthestDoor(array[j], "Top", "Top", false, false) != null && FindFurthestDoor(array[j], "Bottom", "Bottom", false, false) != null)
						{
							break;
						}
				...
				while (k < list3.Count)
				{
					...
					while (true)
					{
						...
						if ((areaInfo2.LevelType != GameTypes.LevelType.TOWER || (FindFurthestDoor(array2[k], "Right", "Right", false, false) != null && FindFurthestDoor(array2[k], "Top", "Top", false, false) != null)) && (areaInfo2.LevelType != GameTypes.LevelType.DUNGEON || (FindFurthestDoor(array2[k], "Right", "Right", false, false) != null && FindFurthestDoor(array2[k], "Bottom", "Bottom", false, false) != null)))
						{
							break;
						}
				...
				if (!(doorPosition == "Left"))
				{
					if (!(doorPosition == "Right"))
					{
						if (!(doorPosition == "Top"))
						{
							if (doorPosition == "Bottom")
							{
								zero = new Vector2(doorToCheck.X - (otherDoorToCheck.X - otherDoorToCheck.Room.X), doorToCheck.Y + doorToCheck.Height);
							}
						}
						else
						{
							zero = new Vector2(doorToCheck.X - (otherDoorToCheck.X - otherDoorToCheck.Room.X), doorToCheck.Y - otherDoorToCheck.Room.Height);
						}
					}
					else
					{
						zero = new Vector2(doorToCheck.X + doorToCheck.Width, doorToCheck.Y - (otherDoorToCheck.Y - otherDoorToCheck.Room.Y));
					}
				}
				else
				{
					zero = new Vector2(doorToCheck.Room.X - otherDoorToCheck.Room.Width, doorToCheck.Y - (otherDoorToCheck.Y - otherDoorToCheck.Room.Y));
				}
			}
			foreach (RoomObj current in roomList)
			{
