...
namespace RogueCastle
{
	internal class LevelBuilder2
	{
		...
		public static RoomObj StartingRoom
		{
			get
			{
				return LevelBuilder2.m_startingRoom;
			}
		}
		...
		public static List<RoomObj> CreateArea(int areaSize, AreaStruct areaInfo, List<RoomObj> roomsToCheckCollisionsList, RoomObj startingRoom, bool firstRoom)
		{
			bool flag = false;
			float num = -100f;
			float num2 = 100f / (float)areaSize;
			...
			if (areaInfo.SecretRooms.Y > (float)list2.Count)
			{
				throw new Exception(string.Concat(new object[]
				{
					"Cannot add ",
			...
			if (areaInfo.BonusRooms.Y > (float)list3.Count)
			{
				throw new Exception(string.Concat(new object[]
				{
					"Cannot add ",
		...
		public static void AddRemoveExtraObjects(List<RoomObj> roomList)
		{
			foreach (RoomObj current in roomList)
			{
				...
				LevelBuilder2.RemoveFromListHelper<TerrainObj>(current.TerrainObjList);
				LevelBuilder2.RemoveFromListHelper<GameObj>(current.GameObjList);
				LevelBuilder2.RemoveFromListHelper<EnemyObj>(current.EnemyList);
				LevelBuilder2.RemoveFromListHelper<BorderObj>(current.BorderList);
			}
		}
		...
		public static ProceduralLevelScreen CreateTutorialRoom()
		{
			...
			LevelBuilder2.MoveRoom(tutorialRoomObj, new Vector2((float)introRoomObj.Width, (float)(-(float)tutorialRoomObj.Height + introRoomObj.Height)));
		...
		public static ProceduralLevelScreen CreateLevel(Vector4[] roomInfoList, Vector3[] roomColorList)
		{
			...
			for (int i = 0; i < roomInfoList.Length; i++)
			{
				...
				list.Add(roomObj);
				roomObj.TextureColor = new Color((int)((byte)roomColorList[num].X), (int)((byte)roomColorList[num].Y), (int)((byte)roomColorList[num].Z));
				num++;
			}
