...
namespace RogueCastle
{
	public class DialogueScreen : Screen
	{
		...
		public override void LoadContent()
		{
			...
			spriteObj.Position = new Vector2(m_dialogContainer.GetChildAt(2).Bounds.Right, m_dialogContainer.GetChildAt(2).Bounds.Bottom);
			...
			if (!m_lockControls && m_inputDelayTimer <= 0f)
			{
					...
					if (Game.GlobalInput.JustPressed(0) || Game.GlobalInput.JustPressed(1) || Game.GlobalInput.JustPressed(2) || Game.GlobalInput.JustPressed(3))
					{
						if (m_dialogCounter < m_dialogText.Length - 1)
						{
							...
							if (!textObj.IsTypewriting)
							{
								...
								textObj.WordWrap(850);
								textObj.BeginTypeWriting(m_dialogText[m_dialogCounter].Length * m_textScrollSpeed, "dialogue_tap");
							}
							else
							{
				...
				else
				{
					if (Game.GlobalInput.JustPressed(18) || Game.GlobalInput.JustPressed(19))
					{
						...
						m_dialogChoiceContainer.GetChildAt(1).Y = m_dialogChoiceContainer.GetChildAt(m_highlightedChoice).Y + m_dialogChoiceContainer.GetChildAt(1).Height / 2 + 3f;
					}
